...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  # Check input errors before inserting data into database
   if (empty($fname_err) && empty($lname_err) && empty($pass_err) && empty($email_err) && empty($gender_err) && empty($designation_err)) {
    ...
    $sql = "SELECT * FROM reservasi where first_name = '$fname' and last_name = '$lname'";
    $result = mysqli_query($link, $sql);
  if (mysqli_num_rows($result) == 0) {
          echo "<script>" . "alert('Tidak ada Anabul $fname dan pawrent $lname Silahkan ke menu Registrasi Untuk membuat akun baru:)');" . "</script>";
        $_SESSION['gagal']=true;
  }else{
    ...
    $sql = "UPDATE reservasi SET  last_name = ? , email= ?, tes = ?, gender = ?, designation = ? WHERE first_name = ?";
    
    if ($stmt = mysqli_prepare($link, $sql)) {
      # Bind variables to the prepared statement as parameters
      mysqli_stmt_bind_param($stmt, "ssssss",  $param_lname, $param_email, $param_tes, $param_gender, $param_designation, $param_fname);
    ...
    <div class="row justify-content-center mt-5">
      ...
      <a href="../" class="w3-button w3-green">BACK</a>
        ...
        <?php }else {?>
          ...
          <h2>Halaman Update data Pengguna</h2>
        ...
        <!-- form starts here -->
        <form action="./cek user.php" class=" p-4 shadow-sm" method="post" novalidate enctype="multipart/form-data">
